...
extern unsigned short int channel;
#include <starlet.h>
#include <iodef.h>
#include <ints.h>
...
extern uint64 mapped_addr_base;
...
{
  printf("sst1InitMapBoard: called for BoardNumber %d\n", BoardNumber);
                                                   ...
                                                   FxBool resetSLI)
{
    ...
    if(firstTime) {
        ...
        for(j = boardLoopStart; j != (FxU32)boardLoopStop; j += boardLoopInc) {
            ...
            printf("hardcoding SST base %08X\n", (FxU32)mapped_addr_base);
            sstbase = (FxU32 *)((FxU32)mapped_addr_base);
            if(sstbase) {
