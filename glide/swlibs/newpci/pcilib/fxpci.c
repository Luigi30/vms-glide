...
#include "fxlinux.h"
#elif (GLIDE_PLATFORM & GLIDE_OS_OPENVMS)
#include "fxovms.h"
#endif
...
{
  printf("calling pciGetConfigData: reg %08X\n", reg.regAddress);
  ...
  sys$qiow(   
    ...
    0,
    data,               // ptr to data
    reg.regAddress,     // region to read from
    4,                  // longword
    TRUE,               // config space
    0,
    0
...
{
  printf("calling pciSetConfigData: reg %08X data %08X\n", reg.regAddress, *data);

  sys$qiow(   
    0,
    channel,            // FXA0:
    IO$_WRITEPBLK,      // command
    NULL,
    0,
    0,
    data,               // ptr to data
